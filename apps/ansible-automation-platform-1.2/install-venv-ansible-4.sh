...
# IMPORTANT:
# - Ansible 4.x is no longer maintained.
# - See install-venv-ansible.README.md for details.

# base dir for the venv; eventually creates "${venv_dir}/ansible-4"
...
# this should be installed already from AAP setup.sh, but Just In Case (tm)...
yum install -y python3

# required by the Python psutil package
yum install -y gcc python3-devel --enablerepo=rhel-7-server-optional-rpms
...
# create venv dir
install -m 0755 -o root -g root -d "${venv_dir}/ansible-4"

# IMPORTANT - mask the permissions to disallow writes for anyone but root
umask 0022

# create the venv if it doesn't exist
if [[ ! -f "${venv_dir}/ansible-4/bin/activate" ]]; then
  python3 -m venv "${venv_dir}/ansible-4"
fi

# activate the venv
# shellcheck source=/dev/null
source "${venv_dir}/ansible-4/bin/activate" \
  || error 'failed to activate venv'

# install/upgrade pip and THEN psutil, ansible, etc
pip install --upgrade pip
pip install --upgrade psutil 'ansible<5'
